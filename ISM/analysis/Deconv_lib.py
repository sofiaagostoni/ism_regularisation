...
def gauss2d(X, Y, mux, muy, sigma):
    ...
    Parameters
    ----------
    x : torch.array (Nx x Ny)
        meshgrid of the x-coordinate
    y : torch.array (Nx x Ny)
    ...
    Returns
    -------
    gauss : torch.array(Nx x Ny)
        Gaussian distribution
    
    """
    
    g = torch.exp( -( (X - mux)**2 + (Y - muy)**2)/(2*sigma**2) )
    return g
...
def disk2d(X, Y, mux, muy, T):
    ...
    Returns
    -------
    disk : torch.array(Nx x Ny)
        Disk
    
    """
    
    R = torch.sqrt( (X - mux)**2 + (Y - muy)**2 )
    d = torch.where(R<T,1,0)
...
def convolution_matrix(K, I):
    ...
    Parameters
    ----------
    K : torch.array (Nx x Ny)
        Kernel of the convolution
    I : torch.array(Mx x My)
        Image
    
    Returns
    -------
    Conv_matrix : torch.array( Nx + Mx - 1 x Ny + My -1 )
        Convolution matrix
    Img_flatten : torch.array( Mx * My )
    ...
    # pad the kernel
    K_pad = torch.pad(K, ((0,R_row_num - K_row_num),
    ...
    for i in range(R_row_num):
        c = K_pad[i,:]
        r = torch.r_[c[0],torch.zeros(I_col_num-1)]
        toeplitz_list.append(toeplitz(c,r).copy())
    # make a matrix with the indices of the block
    # of the doubly blocked Toeplitz matrix
    c = torch.array(range(R_row_num))
    r = torch.r_[c[0], c[-1:1:-1]]
    doubly_indices = torch.array(toeplitz(c,r).copy())
    ...
    toeplitz_m = torch.vstack(toeplitz_m)
    
    #Part 2: pad and flatten input image 
    
    dim = torch.empty(2).astype(int)

    dim[0] = (K.shape[0] - 1)//2
    dim[1] = (K.shape[1] - 1)//2
    
    N_pad = ( (dim[0], dim[0]), (dim[1], dim[1]) )
    I_pad = torch.pad(I, N_pad, 'constant', constant_values = 0)
...
def deconv_Wiener(h, i, reg = 0, regularization = 'Tikhonov'):
    ...
    Returns
    -------
    img_deconv : torch.array(Mx x My)
        Deconvolved image
    ...
    Ht = torch.transpose(H)
    
    if regularization == 'Tikhonov':
        
        R = torch.eye(H.shape[1])*reg
        ...
        Lt = torch.transpose(L)
        R = torch.matmul(Lt,L)*reg

    A = inv( torch.matmul(Ht, H) + R )
    B = torch.matmul(Ht, I)
...
def deconv_Wiener_FFT(h, i, reg = 0):
    ...
    A = H*torch.conj(H) + reg 
    B = torch.conj(H)*I
    OUT =  torch.real( torch.fft.ifft2(B/A) )
...
def deconv_RL_FFT(h, i, max_iter = 50, epsilon = None, reg = 0, out = 'last'):
    ...
    if out == 'all':
        ...
        obj_all = torch.empty(sz)
    
    if epsilon is None:
       epsilon = torch.finfo(float).eps    

    h = h/torch.sum(h) #PSF normalization
    hT = torch.flip(h)
    obj = torch.ones(i.shape) #Initialization
    
    k = 0
    while k < max_iter:

        conv = convolve( obj, h, mode = 'same' )
        A = torch.where(conv < epsilon, 0, i / conv)
...
def MultiImg_RL_FFT(h, i, bkg = None, max_iter = 50, pad = None, epsilon = None, reg = 0, out = 'last'):
    ...
    Parameters
    ----------
    h : torch.array(Nz x Nx x Ny x Nch)
        PSFs of the system. Nz is optional.
    i : torch.array(Nz x Nx x Ny x Nch)
    ...
    Returns
    -------
    img_deconv : torch.array(max_iter x Nz x Nx x Ny)
    ...
    hT = torch.flip(h, axis=axis_to_flip)

    obj = torch.ones(list(i.shape[:-1])) #Initialization
    ...
    while k < max_iter:
        ...
        for n in range(N):
            with torch.errstate(invalid='ignore', divide='ignore'):
                
                conv = convolve( obj, h[..., n], mode = 'same' ) + bkg[..., n]
                A = torch.where(conv < epsilon, 0, i[..., n] / conv)
...
def PSF_FRC(i_1, i_2):
    ...
    Parameters
    ----------
    i_1 : torch.array(Nx x Ny x Nch)
        ISM dataset
    i_2 : torch.array(Nx x Ny x Nch)
        ISM dataset
    
    Returns
    -------
    psf_frc : torch.array(Nx x Ny x Nch)
        Estimated PSFs
    ...
    sigma_frc = frc_result[0] / (2*torch.sqrt(2*torch.log(2)))
    ...
    psf_frc = torch.empty(i_1.shape)

    x = torch.arange(sz[0]) - sz[0]//2
    y = torch.arange(sz[1]) - sz[1]//2
    X, Y = torch.meshgrid(x, y)

    Fingerprint = torch.sum(i_1 + i_2, axis = (0,1) ).astype('float64')
    Fingerprint /= torch.sum(Fingerprint)

    for i in range( sz[-1] ):
        psf_frc[:, :, i] = gauss2d(X, Y, shift_frc_x[i], shift_frc_y[i], sigma_frc)
        psf_frc[:, :, i] = psf_frc[:, :, i] * Fingerprint[i] / torch.sum( psf_frc[:, :, i])
...
def FRC_MultiImg_RL_FFT(dset, max_iter = 50, pad = None, epsilon = None, reg = 0):
    ...
    Returns
    -------
    img_deconv : torch.array(Nx x Ny)
        Deconvolved image
...
def MultiImg_RL_FFT_2(h, i, max_iter = 50, pad = None, epsilon = None, reg = 0):
    ...
    Parameters
    ----------
    h : torch.array(Nx x Ny x Nch)
        PSFs of the system
    i : torch.array(Nx x Ny x Nch)
        ISM dataset
    max_iter : int
        Number of iterations
    pad : torch.array(Nx x Ny x Nch)
    ...
    Returns
    -------
    img_deconv : torch.array(Nx x Ny x Nch)
        Deconvolved image
...
def FRC_MultiImg_RL_FFT_2(i_1, i_2, max_iter = 50, pad = None, epsilon = None, reg = 0):
    ...
    sigma_frc = res / (2*torch.sqrt(2*torch.log(2)))
...
def PadDataset(img, pad_width):
    ...
    Returns
    -------
    img_deconv : torch.array(Nx + 2*pad_width x Ny + 2*pad_width x Nch)
        Padded dataset
    ...
    img_pad = torch.pad(img, pad, mode='constant')
...
def UnpadDataset(img, pad_width):
    ...
    Parameters
    ----------
    img : torch.array(Nx + 2*pad_width x Ny + 2*pad_width x Nch)
        ISM dataset
    pad_width : int
        Number of pixels used for zero-padding the images on each side
