...
_hot_array = _hot(torch.linspace(0, 1, 256))
...
#%%
def ShowImg(image: torch.tensor, pxsize_x: float, clabel: str = None, vmin: float = None, vmax: float = None,
    ...
    if log_scale == True:
        im = ax.imshow(image.squeeze(), vmin=vmin, vmax=vmax, cmap=cmap, norm = LogNorm())
    else:
        im = ax.imshow(image.squeeze(), vmin=vmin, vmax=vmax, cmap=cmap)#, extent=extent)
    ax.axis('off')

    if vmax is None:
        vmax = torch.max(image[~torch.isnan(image)])
    if vmin is None:
        vmin = torch.min(image[~torch.isnan(image)])
    ...
    if colorbar==True:
        ...
        if integer_values == True:
            vmax_text = int(torch.floor(vmax))
            vmin_text = int(torch.floor(vmin))
        else:
            vmax_text = torch.round(vmax, 3)
            vmin_text = torch.round(vmin, 3)
...
def ShowStack(image: torch.tensor, pxsize_x: float, pxsize_z: float, clabel: str = None, planes: tuple = None,
    ...
    if vmax is None:
        max3d = torch.empty(image.ndim)

        max3d[0] = torch.max(image_z)
        max3d[1] = torch.max(image_y)
        max3d[2] = torch.max(image_x)

        vmax = torch.max(max3d)

    # find vmin
    if vmin is None:
        min3d = torch.empty(image.ndim)

        min3d[0] = torch.min(image_z)
        min3d[1] = torch.min(image_y)
        min3d[2] = torch.min(image_x)

        vmin = torch.max(min3d)
    ...
    ax = torch.asarray([plt.subplot(gs[i]) for i in range(4)]).reshape((2,2))
...
def StackSlider(image: torch.tensor, pxsize_x: float, pxsize_z: float, clabel: str = None,
    ...
    def update_y(val):
        ...
        seg_x = [torch.array([[-rangex/2, y_units],
                         [rangex/2, y_units]])]
    ...
    def update_x(val):
        ...
        seg_y = [torch.array([[x_units, -rangey/2],
                         [x_units, rangey/2]])]
    ...
    def update_z(val):
        ...
        seg_zx = [torch.array([[z_units, -rangey/2],
                         [z_units, rangey/2]])]

        line_zx.set_segments( seg_zx )

        seg_zy = [torch.array([[-rangex/2, z_units],
                         [rangex/2, z_units]])]
...
def ShowDataset(dset: torch.tensor, cmap: str = 'hot', pxsize: float = None, normalize: bool = True,
    ...
    dset = dset.permute(1, 2, 3, 0)
    dset = dset.squeeze()

    if gridshape is None:
        nx = int(torch.sqrt(torch.tensor(dset.shape[-1], dtype=torch.float32)))
        ny = int(torch.sqrt(torch.tensor(dset.shape[-1], dtype=torch.float32)))
    else:
        nx = gridshape[0]
        ny = gridshape[1]
    ...
    for i in range(nx*ny):
        if min(nx, ny) > 1:
            idx = torch.unravel_index(torch.tensor(i), [nx, ny])
        else:
            idx = i
        if normalize == True:
    ...
    if colorbar == True and normalize == True:
        ...
        cbar.ax.text(0.3, 0.95, f'{int(torch.floor(vmax))}', rotation=90, transform=cbar_ax.transAxes)

        cbar.ax.text(0.3, 0.02, f'{int(torch.floor(vmin))}', rotation=90, transform=cbar_ax.transAxes, color='white')

    return fig


def PlotShiftVectors(shift_vectors: torch.tensor, pxsize: float = 1, labels: bool = True, color: torch.tensor = None,
...
def ShowFingerprint(dset: torch.tensor, cmap: str = 'hot', colorbar: bool = False, clabel: str = None, normalize: bool = False, fig: plt.Figure = None,
    ...
    if name == 'airyscan':
        ...
        N = int(torch.ceil(torch.sqrt(dset.shape[-1])))

    if normalize is True:
        max_counts = torch.nanmax(fingerprint)
        fingerprint = fingerprint / max_counts

    if hex_grid is True:
        k = (N-1)/2
        gridsize = torch.asarray([N - (1+(-1)**k)/2, N//2]).astype(int)
        if name == 'airyscan':
            gridsize[0] -= 1

        s = -det_coords(N, 'hex')
        s = torch.flip(s, axis=0)
    ...
    if colorbar is True:
        ...
        vmax = int(torch.floor(torch.max(fingerprint)))
        vmin = int(torch.floor(torch.min(fingerprint)))
...
class ColorMap2D:
    ...
    def image(self, intensity_map, variable_map):
        output = torch.zeros(intensity_map.shape + (3,))

        intensity = torch.clip(intensity_map, self.int_bounds[0], self.int_bounds[1])
        intensity = (intensity - self.int_bounds[0]) / (self.int_bounds[1] - self.int_bounds[0])

        cmap = plt.get_cmap(self.colormap)
        N = cmap.N
        cmapArray = torch.zeros((N, 3))
        ...
        if self.equalize:
            lmax = torch.max((299 * cmapArray[:, 0] + 587 * cmapArray[:, 1] + 114 * cmapArray[:, 2]) / 1000)
            for i in range(N):
                lum = (299 * cmapArray[i, 0] + 587 * cmapArray[i, 1] + 114 * cmapArray[i, 2]) / 1000
                lumfactor = torch.min((lmax / lum, 1 / torch.max(cmapArray[i, :])))
                cmapArray[i, :] *= lumfactor

        # get index of color map for lifetime
        variable = torch.clip(variable_map, self.var_bounds[0], self.var_bounds[1])
        idx = (torch.floor((variable - self.var_bounds[0]) / (self.var_bounds[1] - self.var_bounds[0]) * N)).astype(int)
        idx = torch.clip(idx, 0, N - 1)
        for k in range(3):
            output[:, :, k] = torch.take(cmapArray[:, k], idx) * intensity

        return output

    def colorbar(self, n):
        LG = torch.linspace(self.var_bounds[0], self.var_bounds[1], num=n)
        VarGradient = torch.tile(LG, (n, 1))

        IG = torch.linspace(self.int_bounds[0], self.int_bounds[1], num=n)
        IntensityGradient = torch.transpose(torch.tile(IG, (n, 1)))

        RGB_colormap = self.image(IntensityGradient, VarGradient)
        RGB_colormap = torch.moveaxis(RGB_colormap, 0, 1)

        return RGB_colormap


def show_flim(image: torch.tensor, lifetime: torch.tensor, pxsize: list, pxdwelltime: float, lifetime_bounds: list = None,
    ...
    if intensity_bounds is None:
        cmap.int_bounds = [torch.min(image), torch.max(image)]
    else:
        cmap.int_bounds = intensity_bounds

    if lifetime_bounds is None:
        cmap.var_bounds = [torch.min(lifetime), torch.max(lifetime)]
    else:
...
def depth_stack(stack: torch.tensor, pxsize: list, pxdwelltime: float, axis: int = 0, colormap='rainbow', fig=None,
                ax=None):
    """
    ...
    cmap.int_bounds = [torch.min(image_mip), torch.max(image_mip)]
    cmap.var_bounds = [torch.min(depth_mip), torch.max(depth_mip)]
