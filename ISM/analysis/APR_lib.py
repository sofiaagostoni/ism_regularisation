...
def hann2d(shape: tuple):
    ...
    W = 0.5 * (1 - torch.cos((2 * torch.pi * X) / (Nx - 1)))
    W *= 0.5 * (1 - torch.cos((2 * torch.pi * Y) / (Ny - 1)))

    return W.T


def APR(dset: torch.tensor, usf: int, ref: int, pxsize: float = 1, apodize: bool = True, filter_sigma: float = 1,
        mode: str = 'interp'):
    '''
...
def ShiftVectors(dset: torch.tensor, usf: int, ref: int, apodize: bool = True, filter_sigma: float = 0):
    '''
    ...
    if apodize == True:
        W = hann2d(dset.shape)
        dsetW = torch.einsum('ijk, ij -> ijk', dsetW, W)
...
def Reassignment(shift_vec: torch.tensor, dset: torch.tensor, mode: str = 'interp'):
    '''
    ...
    sz = dset.shape
    result_ism_pc = torch.empty(sz)

    for i in range(sz[-1]):
        if mode == 'fourier':
            offset = fourier_shift(torch.fft.fftn(dset[:, :, i]), (shift_vec[i, :]))
            result_ism_pc[:, :, i] = torch.real(torch.fft.ifftn(offset))
        elif mode == 'interp':
