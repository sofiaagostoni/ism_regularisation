...
def circle(n, radius):
    ...
    Returns
    -------
    circle_pinhole : np.tensor (n x n)
        Image of the binary mask.
    """

    x = torch.arange(-(n // 2), n // 2 + n % 2)

    xx, yy = torch.meshgrid(x, x)
    r = torch.sqrt(xx ** 2 + yy ** 2)

    circle_pinhole = torch.where(r < radius, 1, 0)
...
def square(n, length):
    ...
    Returns
    -------
    square_pinhole : torch.tensor (n x n)
        Image of the binary mask.
    """
    ...
    cond = torch.logical_and(torch.abs(xx) < length / 2, torch.abs(yy) < length / 2)

    square_pinhole = torch.where(cond, 1, 0)
...
def hexagon(n, radius):
    ...
    Returns
    -------
    hex_pinhole : torch.tensor (n x n)
        Image of the binary mask.
    """

    x = torch.tensor([-1, 0, 1, 1, 0, -1]) * radius * torch.cos(torch.deg2rad(30)) + n // 2
    y = torch.tensor([1, 2, 1, -1, -2, -1]) * radius * torch.sin(torch.deg2rad(30)) + n // 2

    hex_pinhole = torch.zeros((n, n))
...
def hex_grid(n, x):
    ...
    for n_i, i in enumerate(x):
        for n_j, j in enumerate(x):
            s[n_i, n_j] = torch.tensor([0.5 * torch.sqrt(3) * i, j - 0.5 * (i % 2)])

    s = s.reshape(-1, 2).T

    condition = torch.abs(s[1]) <= (n // 2)
    idx = torch.argwhere(condition).flatten()
    s = s[:, idx]
...
def pinhole_array(s, nx, mag, pxsize, pxdim, pinhole_shape, device):
    ...
    Returns
    -------
    detector : torch.tensor (Nx x Nx x Nch)
    ...
    pinhole = torch.tensor(pinhole).to(device)
...
def transform_detector(gridPar, detector):
    ...
    if gridPar.rotation != 0:
        theta = torch.rad2deg(gridPar.rotation)
...
def custom_detector(grid, device):
    ...
    s = torch.round(s / grid.M / grid.pxsizex).to(torch.int)
...
def airy_to_hex(s):
    ss = torch.ones((s.shape[:-1] + (45,))) * torch.nan
