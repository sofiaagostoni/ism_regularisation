...
class tubSettings:
    ...
    def __init__(self):
        ...
        self.n_filament = 2               # number of filaments (but it can be also random with an interval of [10,100] )
...
def thetaVariation(z):
    ...
    initial_theta_xy = 0
    initial_theta_variation_xy = 2*torch.pi/20  
    theta_variation_xy = 2*torch.pi/50         
    theta_xy_boundaries = torch.pi/4   
    ...
    if z == 1:
        ...
        initial_theta_xz = 0
        initial_theta_variation_xz = 2*torch.pi/100  
        theta_variation_xz = torch.pi/50
...
def functionPhTub(tub: tubSettings):
    ...
    phTub = torch.zeros((tub.xy_dimension,tub.xy_dimension,tub.xz_dimension)) #Phantom initialization 
    ...
    cIn, rIn, dIn = torch.meshgrid(torch.arange(0,tub.xy_dimension), torch.arange(0,tub.xy_dimension), torch.arange(1,tub.xz_dimension+1))
    ...
    for i in tqdm(range(0,tub.n_filament)):
        ...
        currIntensity = tub.intensity_filament[0] + (tub.intensity_filament[1] - tub.intensity_filament[0])*torch.rand(1,1)    
        theta_xy = xy_0 - xy_0_var + 2 * xy_0_var * torch.rand(1)
        theta_xz = xz_0 - xz_0_var + 2 * xz_0_var * torch.rand(1)
        
        xyC = torch.randint(0,tub.xy_dimension, (2,))
        yC = xyC[1]
        xC = 1
        zC = torch.randint(0,tub.xz_dimension,(1,))
        zC = math.ceil(tub.xz_dimension/2)
        
        elipsoid_vol = torch.zeros((tub.xy_dimension,tub.xy_dimension,tub.xz_dimension)) 
        ...
        #       Calculate the next coord
            next_theta_xy  = theta_xy - xy_var + 2 * xy_var * torch.rand(1)
            next_theta_xz  = theta_xz - xz_var + 2 * xz_var * torch.rand(1)
            next_xC = xC + xR * torch.cos(next_theta_xy)
            next_yC = yC + yR * torch.sin(next_theta_xy)
            next_zC = zC + zR * torch.sin(next_theta_xz)
            ...
            if next_zC < 0 or next_zC > tub.xz_dimension:
                ...
                theta_xz = next_theta_xz + torch.pi
                xC = next_xC
                yC = next_yC

            if next_theta_xy < -xy_bound or next_theta_xy > xy_bound :
                
                theta_xy =  next_theta_xy - xy_var * torch.sign(next_theta_xy)
